...
def conv2d(inputs, thetas):
    return tf.nn.conv2d(inputs, thetas, strides=[1, 1, 1, 1], padding='SAME', data_format='NCHW')


def layer(inputs, filter_size, n_channels, dtype):
    thetas = weight_variable([filter_size, filter_size, n_channels, n_channels], dtype)
    #thetas = weight_variable([1, n_output_channels, filter_size, filter_size], dtype)
    # biases = bias_variable([n_channels], dtype)
    return conv2d(inputs, thetas) # + biases


class Parameters:

    def __init__(self, dtype=None, runsize=None, nsteps=None, nin=None, nchannels=None, layers=None, filter_size=None):
        self.dtype = dtype
...
def build_inputs(args):
    ...
    x_image_initializer = tf.random_normal(shape=(args.runsize, args.nchannels, args.nin, args.nin), dtype=args.dtype)
...
def build_model(args, x_image):
    ...
    for i in range(args.layers):
        out = layer(outputs[-1], args.filter_size, args.nchannels, args.dtype)
        outputs.append(out)
    return outputs[-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nchannels", type=int, default=default_nchannels,
                        help='Channels size of the layer (default %d)' % default_nchannels)
    ...
    for dtype in tested_dtypes:
        parameters = Parameters(dtype=dtype, runsize=runsize, nsteps=args.nsteps, nin=args.nin, nchannels=args.nchannels,
